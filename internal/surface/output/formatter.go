...
func ResolveFormat(cmd *cobra.Command, fallback Format) Format {
	...
	for _, flags := range []*pflag.FlagSet{cmd.Flags(), cmd.InheritedFlags()} {
		if format, ok := formatFromFlagSet(flags, OutputFormatFlag, fallback); ok {
			return format
		}
	}
	if root := cmd.Root(); root != nil {
		if format, ok := formatFromFlagSet(root.PersistentFlags(), OutputFormatFlag, fallback); ok {
			return format
		}
	}

	if cmd.LocalFlags().Lookup(LegacyFormatFlag) == nil {
		for _, flags := range []*pflag.FlagSet{cmd.Flags(), cmd.InheritedFlags()} {
			if format, ok := formatFromFlagSet(flags, LegacyFormatFlag, fallback); ok {
				return format
			}
		}
	}
	if root := cmd.Root(); root != nil {
		if format, ok := formatFromFlagSet(root.PersistentFlags(), LegacyFormatFlag, fallback); ok {
			return format
		}
	}
	return fallback
...
func formatFromFlagSet(flags *pflag.FlagSet, name string, fallback Format) (Format, bool) {
	if flags == nil {
