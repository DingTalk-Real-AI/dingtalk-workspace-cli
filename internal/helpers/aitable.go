...
func runAitableViewUpdateFilter(cmd *cobra.Command) error {
	...
	_, err = callMCPToolReturnTextOnServer(cmd.Context(), "aitable", "update_view", toolArgs)
	if err != nil {
		return err
	}
	var actual any
	...
	for attempt := 0; attempt < aitableViewFilterReadbackAttempts; attempt++ {
		...
		matched, unknown, comparedActual := compareAitableViewFilterReadBack(view, filter, fieldTypes)
