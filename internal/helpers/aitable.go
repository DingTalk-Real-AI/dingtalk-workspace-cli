...
// parseJSONObjectFlag strictly parses flags whose contract requires a JSON object.
func parseJSONObjectFlag(flagName, raw string) (map[string]any, error) {
	var parsed any
	if err := json.Unmarshal([]byte(raw), &parsed); err != nil {
		return nil, fmt.Errorf("--%s JSON parse failed: %w\n  hint: --%s must be a JSON object like '{\"key\":\"value\"}'", flagName, err, flagName)
	}
	object, ok := parsed.(map[string]any)
	if !ok || object == nil {
		return nil, fmt.Errorf("--%s must be a JSON object, got %T", flagName, parsed)
	}
	return object, nil
}

var knownCreateViewConfigKeys = map[string]bool{
...
var createViewConfigKeyHints = map[string]string{
	"description":        "视图描述请使用独立参数 --desc",
	"fieldWidths":        "请先创建视图，再使用 dws aitable view update field-widths",
	"aggregate":          "请先创建视图，再使用 dws aitable view update aggregate",
	"kanbanCard":         "请先创建视图，再使用 dws aitable view update card",
	"ganttTimebar":       "请先创建视图，再使用 dws aitable view update timebar",
	"galleryCard":        "请先创建视图，再使用 dws aitable view update card",
	"flags":              "请先创建视图，再使用 dws aitable view lock",
	"frozenColCount":     "请先创建视图，再使用 dws aitable view update frozen-cols",
	"cellHeight":         "请先创建视图，再使用 dws aitable view update row-height",
	"conditionalFormats": "请先创建视图，再使用 dws aitable view update fill-color-rule",
}

// normalizeCreateViewConfigBlock validates the narrower create-view contract.
func normalizeCreateViewConfigBlock(cfgMap map[string]any) error {
	var unsupportedKeys []string
	for key := range cfgMap {
		if !knownCreateViewConfigKeys[key] {
	...
	if len(unsupportedKeys) > 0 {
		...
		for _, key := range unsupportedKeys {
			if hint, ok := createViewConfigKeyHints[key]; ok {
				hints = append(hints, fmt.Sprintf("%s: %s", key, hint))
			}
		}
		message := fmt.Sprintf(
			"unsupported --config key(s) for view create: %s\n  supported keys: visibleFieldIds, filter, sort, group",
	...
	if rawFieldIDs, ok := cfgMap["visibleFieldIds"]; ok {
		var fieldIDs []string
		switch values := rawFieldIDs.(type) {
		case []string:
			fieldIDs = make([]string, 0, len(values))
			for index, value := range values {
				fieldID := strings.TrimSpace(value)
				if fieldID == "" {
					return fmt.Errorf("invalid config.visibleFieldIds[%d]: must be a non-empty fieldId string", index)
				}
		...
		case []any:
			fieldIDs = make([]string, 0, len(values))
			for index, value := range values {
				rawFieldID, ok := value.(string)
				fieldID := strings.TrimSpace(rawFieldID)
				if !ok || fieldID == "" {
					return fmt.Errorf("invalid config.visibleFieldIds[%d]: must be a non-empty fieldId string", index)
				}
				fieldIDs = append(fieldIDs, fieldID)
		...
		default:
			return fmt.Errorf("invalid config.visibleFieldIds: must be a JSON string array, got %T", rawFieldIDs)
		}
		if len(fieldIDs) == 0 {
			return fmt.Errorf("invalid config.visibleFieldIds: must contain at least the primary fieldId")
		}
		cfgMap["visibleFieldIds"] = fieldIDs
	}

	return normalizeViewConfigBlock(cfgMap)
}
...
// resolveFieldCreateInputs preserves the historical batch-first precedence:
// when --fields is non-empty, single-field shorthand flags are ignored.
func resolveFieldCreateInputs(cmd *cobra.Command) (fieldsJSON, name, fieldType string, batchMode bool, err error) {
	fieldsJSON, _ = cmd.Flags().GetString("fields")
	if fieldsJSON != "" {
		return fieldsJSON, "", "", true, nil
	}

	singleModeFlagNames := []string{
		"name", "field-name", "type", "field-type", "config", "ai-config", "options",
	}
	var changedSingleModeFlags []string
	for _, flagName := range singleModeFlagNames {
		if cmd.Flags().Changed(flagName) {
			changedSingleModeFlags = append(changedSingleModeFlags, "--"+flagName)
		}
	}
	name, _ = cmd.Flags().GetString("name")
	if name == "" {
		name, _ = cmd.Flags().GetString("field-name")
	}
	fieldType, _ = cmd.Flags().GetString("type")
	if fieldType == "" {
		fieldType, _ = cmd.Flags().GetString("field-type")
	}
	if name == "" || fieldType == "" {
		if name == "" && fieldType == "" && len(changedSingleModeFlags) == 0 {
			return "", "", "", false, fmt.Errorf("must specify either --fields OR both --name and --type")
		}
		return "", "", "", false, fmt.Errorf(
			"single-field mode requires both --name and --type; --config, --ai-config, and --options cannot be used without them")
	}

	return "", name, fieldType, false, nil
}
...
--base-id (必填)：Base ID，可通过 dws aitable base list 或 dws aitable base search 获取`,
  ...
  # 查询 tableId: dws aitable table get --base-id <baseId>`,
	...
	fieldCreateCmd := &cobra.Command{
		Use:   "create",
		Short: "创建字段",
		Long: `在已有表格中新增字段，支持两种输入模式：
  批量模式：传 --fields，字段配置放在数组内每个字段对象的 config/aiConfig 中。
  单字段模式：传 --name 和 --type，可附加 --config、--ai-config。
为兼容既有调用，同时传入两种模式的参数时以 --fields 为准，单字段参数会被忽略。

适用于建表后补充一批字段，或一次性添加多个关联、流转等复杂类型字段。
...
每个字段对象包含：
  ...
  # 查询 tableId: dws aitable table get --base-id <baseId>`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var fields []any
			fieldsStr, name, typ, batchMode, err := resolveFieldCreateInputs(cmd)
			if err != nil {
				return err
			}
...
每条记录必须包含 recordId 和 cells 字段。cells 的 value 格式与 record create 相同（详见 record create --help）。
cells 的 key 支持 fieldId 或字段名：
  - 推荐使用 fieldId，不受字段重命名或重名影响；可通过 table get / field get 获取。
  - 字段名按当前表内的名称精确匹配，且必须唯一；存在重名时请改用 fieldId。
  - 同一字段同时通过 fieldId 和字段名传入时，fieldId 的值优先。
...
Windows 用户注意：如果 --records JSON 很长，请使用 --records-file 参数指定一个 JSON 文件路径。`,
	  ...
	  dws aitable record update --base-id BASE_ID --table-id TABLE_ID --records '[{"recordId":"recXXX","cells":{"状态":"已完成"}}]'
	  dws aitable record update --base-id BASE_ID --table-id TABLE_ID --records-file ./data/updates.json
	  # 查询 baseId: dws aitable base list
	  # 查询 tableId / fieldId: dws aitable table get --base-id <baseId>`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := validateRequiredFlags(cmd, "table-id"); err != nil {
				return err
			}
...
创建时 --config 只支持 visibleFieldIds、filter、sort、group 四个 key：
  - visibleFieldIds: fieldId 字符串数组，控制可见字段及顺序；必须把主字段放在第一位。
  - filter/sort/group: 视图筛选、排序、分组规则数组。
视图描述使用独立的 --desc 参数，不能放进 --config。
如果只需设置可见字段，可使用 --visible-field-ids 的 CSV 写法，避免手写 JSON。`,
	  ...
	  dws aitable view create --base-id BASE_ID --table-id TABLE_ID --view-type Grid --visible-field-ids fldPrimary,fldStatus,fldOwner
	  dws aitable view create --base-id BASE_ID --table-id TABLE_ID --view-type Grid --config '{"visibleFieldIds":["fldPrimary","fldStatus"],"sort":[{"fieldId":"fldStatus","direction":"asc"}]}'
	  # 查询 baseId: dws aitable base list
	  # 查询 tableId / fieldId: dws aitable table get --base-id <baseId>`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if rawConfig, _ := cmd.Flags().GetString("config"); rawConfig != "" || cmd.Flags().Changed("config") {
				config, err = parseJSONObjectFlag("config", rawConfig)
				if err != nil {
					return err
				}
			}
			if cmd.Flags().Changed("visible-field-ids") {
				rawFieldIDs, _ := cmd.Flags().GetString("visible-field-ids")
				fieldIDs := parseCSVValues(rawFieldIDs)
				if len(fieldIDs) == 0 {
					return fmt.Errorf("--visible-field-ids must contain at least the primary fieldId")
				}
				if config == nil {
					config = map[string]any{}
				}
				if _, exists := config["visibleFieldIds"]; exists {
					return fmt.Errorf("--visible-field-ids cannot be combined with config.visibleFieldIds; choose one input form")
				}
				config["visibleFieldIds"] = fieldIDs
			}
			if config != nil {
				if err := normalizeCreateViewConfigBlock(config); err != nil {
					return err
				}
...
这是 dws aitable field get 的便捷别名，两个命令完全等价。`,
  ...
  dws aitable field list --base-id BASE_ID --table-id TABLE_ID --field-ids fld1,fld2`,
	...
	recordUpdateCmd.Flags().String("records", "", "待更新的记录内容列表 JSON 数组，单次最多 100 条；cells key 支持 fieldId 或当前表内唯一字段名，推荐 fieldId (必填)")
	...
	recordUpdateCmd.Flags().String("cells", "", "单条记录的 cells JSON 对象；key 支持 fieldId 或当前表内唯一字段名，推荐 fieldId（与 --record-id 配合使用，自动构造 --records）")
...
需在 --field-ids 中显式指定字段 ID 才会返回。`,
  ...
  dws aitable record get --base-id BASE_ID --table-id TABLE_ID --record-ids rec1 --field-ids fldFormulaId,fldLookupId`,
	...
	viewCreateCmd.Flags().String("config", "", "创建视图配置 JSON，仅支持 visibleFieldIds、filter、sort、group；description 请使用 --desc")
	viewCreateCmd.Flags().String("visible-field-ids", "", "创建视图时的可见字段及顺序 fieldId 列表（CSV）；主字段必须排第一；与 config.visibleFieldIds 互斥")
