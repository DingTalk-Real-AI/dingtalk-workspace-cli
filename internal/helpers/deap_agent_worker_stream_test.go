...
func TestCrossPlatformCoverageEmployeeWorkerStreamFailures(t *testing.T) {
	for _, scenario := range []string{"prepare", "executor-state", "audit", "ledger", "command", "transport-state", "running-state", "exit-before-ready", "stderr-oversized", "stdout-oversized", "exit-after-ready", "unobserved", "transport-write", "enqueue", "fatal", "cancel-start", "cancel-running", "hang-exit"} {
		t.Run(scenario, func(t *testing.T) {
			...
			defer cancel()
			graceful := scenario == "cancel-start" || scenario == "cancel-running" || scenario == "hang-exit"
			if graceful {
