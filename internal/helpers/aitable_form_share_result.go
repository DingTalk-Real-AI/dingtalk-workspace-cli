...
const aitableFormSharePartialResultSchema = `{
  "type":"object",
  "description":"终态未验证：ok=false、outcome=partial_failure、退出码7。succeeded仅表示已收到远端响应，不表示配置或CP已成功；原始data保留在succeeded[0].response。failed[0].error.execution_started=true，不自动重放写入。",
  "properties":{
...
// shortcut writes. Receiving a remote response is not proof of CP convergence.
func AitableFormShareUpdateResult(data any) output.CommandResult {
	invalid := invalidFormShareStateFields(data)
	if len(invalid) == 0 {
...
func invalidFormShareStateFields(raw any) []string {
