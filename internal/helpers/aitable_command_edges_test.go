...
func TestCrossPlatformCoverageAitableViewFilterValidationAndReadBack(t *testing.T) {
	...
	t.Run("group open conversation id uses complete external read-back for verification", func(t *testing.T) {
		...
		readBack := `{"data":{"views":[{"viewId":"view","viewType":"Grid","filter":{"operator":"and","operands":[{"operator":"eq","operands":["fldGroup","cid-1"]}]},"filterExternalComplete":true,"filterExternal":{"operator":"and","operands":[{"operator":"eq","operands":["fldGroup",{"openConversationId":"open-cid-1"}]}]}}]}}`
