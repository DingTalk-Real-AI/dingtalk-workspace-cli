...
func TestCrossPlatformCoverageChatIMCanonicalFlagsHideLegacyGroupAlias(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "message edit exposes message-id",
			args:     []string{"message", "edit", "--help"},
			contains: []string{"--message-id"},
			hidden:   []string{"--msg-id", "--open-message-id"},
		},
		{
			name:     "message update text emotion exposes message-id",
			args:     []string{"message", "update-text-emotion", "--help"},
			contains: []string{"--message-id"},
			hidden:   []string{"--msg-id", "--open-message-id"},
		},
	}
