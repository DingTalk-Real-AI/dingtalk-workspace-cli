...
// HintInterfaceRef links a stable public Schema tool to the MCP interface
// operation that implements it. Canonical paths remain stable even when a
// helper uses a friendlier local name than the upstream RPC.
type HintInterfaceRef struct {
...
type HintTool struct {
	...
	SourceRefs    []string          `json:"source_refs,omitempty"`
	InterfaceRef  *HintInterfaceRef `json:"interface_ref,omitempty"`
}
