...
func TestValidateRepoTargetAllowlist(t *testing.T) {
	root := t.TempDir()
	allowed := filepath.Join(root, "internal/cli/schema_catalog")
	if err := os.MkdirAll(filepath.Dir(allowed), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := ValidateRepoTargetAllowlist(root, Target{Name: "--output", Path: allowed}, "internal/cli/schema_catalog"); err != nil {
		t.Fatalf("canonical output rejected: %v", err)
	}
	err := ValidateRepoTargetAllowlist(root, Target{Name: "--output", Path: filepath.Join(root, "skills/mono/SKILL.md")}, "internal/cli/schema_catalog")
