...
func (c *Client) fetchServersPage(ctx context.Context, limit int, cursor string) (ListResponse, error) {
	...
	slog.Debug("market fetchServersPage", "url", reqURL.String(), "limit", limit, "cursor", cursor)
	...
	if err != nil {
		slog.Debug("market fetchServersPage failed", "url", reqURL.String(), "elapsed", elapsed, "error", err)
		return ListResponse{}, apperrors.NewDiscovery(fmt.Sprintf("market servers request failed: %v", err))
	}
	defer resp.Body.Close()

	slog.Debug("market fetchServersPage response", "url", reqURL.String(), "status", resp.StatusCode, "elapsed", elapsed)
	...
	slog.Debug("market fetchServersPage decoded", "servers", len(payload.Servers), "next_cursor", payload.Metadata.NextCursor)
	return payload, nil
}
...
func NormalizeServers(response ListResponse, source string) []ServerDescriptor {
	...
	for _, envelope := range response.Servers {
		...
		descriptor := ServerDescriptor{
			...
			CLI:        cliMeta,
			HasCLIMeta: hasCLIMetadata(cliMeta),
		}
...
func hasCLIMetadata(cli CLIOverlay) bool {
	if strings.TrimSpace(cli.ID) != "" || strings.TrimSpace(cli.Command) != "" {
		return true
	}
	if strings.TrimSpace(cli.Description) != "" || strings.TrimSpace(cli.Group) != "" {
		return true
	}
	if cli.Skip || cli.Hidden {
		return true
	}
	return len(cli.Prefixes) > 0 ||
		len(cli.Aliases) > 0 ||
		len(cli.Tools) > 0 ||
		len(cli.Groups) > 0 ||
		len(cli.ToolOverrides) > 0
}
