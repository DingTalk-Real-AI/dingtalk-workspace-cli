...
const (
	...
	// JSON-RPC response to prevent memory exhaustion from malicious servers.
...
func (r *ToolCallResult) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(raw.Content, &object); err == nil {
		r.Content = normalizeToolContentMap(object)
		return nil
	}
	...
	if err := json.Unmarshal(raw.Content, &blocks); err == nil {
		...
		for _, block := range blocks {
			...
			if err := json.Unmarshal([]byte(text), &parsed); err == nil {
				r.Content = normalizeToolContentMap(parsed)
				return nil
			}
		}
		return nil
...
func normalizeToolContentMap(src map[string]any) map[string]any {
	if len(src) == 0 {
		return nil
	}
	out := make(map[string]any, len(src))
	for key, value := range src {
		out[key] = normalizeToolContentValue(key, value)
	}
	return out
}

func normalizeToolContentValue(key string, value any) any {
	switch typed := value.(type) {
	case map[string]any:
		return normalizeToolContentMap(typed)
	case []any:
		out := make([]any, len(typed))
		for i, item := range typed {
			out[i] = normalizeToolContentValue("", item)
		}
		return out
	case string:
		if looksBooleanResultField(key) {
...
func looksBooleanResultField(name string) bool {
...
func (c *Client) callJSONRPC(ctx context.Context, endpoint string, request requestEnvelope, expectResponse bool, out any) error {
	...
	slog.Debug("jsonrpc request", "method", request.Method, "endpoint", RedactURL(endpoint), "body", truncateDebug(string(body), 512))
	...
	if err != nil {
		slog.Debug("jsonrpc request failed", "method", request.Method, "endpoint", RedactURL(endpoint), "elapsed", elapsed, "error", err)
		return err
	}
	defer resp.Body.Close()

	data, err := io.ReadAll(io.LimitReader(resp.Body, maxResponseBodySize))
	logging.LogResponse(c.FileLogger, request.Method, endpoint, resp.StatusCode, len(data), elapsed, err)
	if err != nil {
	...
	slog.Debug("jsonrpc response", "method", request.Method, "endpoint", RedactURL(endpoint), "status", resp.StatusCode, "elapsed", elapsed, "response_bytes", len(data), "response", truncateDebug(string(data), 256))
...
// truncateDebug truncates a string to maxLen characters for debug logging.
// If the string exceeds maxLen, it appends "...(N more)" to indicate truncation.
func truncateDebug(value string, maxLen int) string {
	if len(value) <= maxLen {
		return value
	}
	return fmt.Sprintf("%s...(%d more)", value[:maxLen], len(value)-maxLen)
}
