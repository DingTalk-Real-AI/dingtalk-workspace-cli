...
func TestToolCallerAdapter_GlobalDryRunAllowsOnlyPATBatchPlanPreview(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			...
			"result": map[string]any{
				"content": map[string]any{"plan": "server-preview"},
			},
		})
	}))
	defer server.Close()
	...
	for _, tt := range blocked {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := calls.Load(); got != 1 {
				t.Fatalf("real call count = %d, want 1", got)
			}
		})
	}
}
...
func TestToolCallerAdapter_ReadOnlyDryRunMarkerIsStrict(t *testing.T) {
	tests := []struct {
		...
		{name: "exact", product: "pat", tool: "pat.batch_plan", args: map[string]any{"dryRun": true}, want: true},
