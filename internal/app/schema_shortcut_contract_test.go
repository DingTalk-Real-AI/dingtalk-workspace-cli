...
func TestDeliveryDocUpdateShortcutPublishesCompleteConditionalContract(t *testing.T) {
	...
	if got, want := len(parameters), 9; got != want {
		t.Fatalf("parameter count = %d, want %d: %#v", got, want, parameters)
	}
	for _, name := range []string{"doc", "text"} {
		if _, exists := parameters[name]; exists {
			t.Errorf("hidden compatibility alias --%s leaked into Schema", name)
		}
	}
	...
	wantProperties := map[string]string{
		"node": "nodeId", "command": "command", "content": "content", "doc-format": "format",
