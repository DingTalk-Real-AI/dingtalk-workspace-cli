...
func newCommandRunnerWithFlags(loader cli.CatalogLoader, flags *GlobalFlags) executor.Runner {
	// Plugin manifests are expanded after runner construction. Preserve the
	// normal cold-start contract by restoring a persisted ClientID before that
	// expansion. The exact PAT chmod preview route is the sole exception because
	// its root construction must not resolve or cache credentials.
...
func (r *runtimeRunner) executeInvocation(ctx context.Context, endpoint string, invocation executor.Invocation) (result executor.Result, retErr error) {
	...
	if IsStdioEndpoint(endpoint) {
		if strictReadOnly {
			return executor.Result{}, apperrors.NewAPI(
				"strict PAT dry-run requires a remote HTTP(S) MCP endpoint; refusing local stdio execution",
				apperrors.WithOperation("tools/call"),
				apperrors.WithReason("pat_strict_dry_run_stdio_forbidden"),
				apperrors.WithHint("修复 PAT endpoint/plugin 配置后重试；严格 dry-run 不会启动本地 stdio 进程。"),
			)
		}
	...
	defer func() {
		...
		emitAuditWithActorPolicy(auditSink, execID, invokeStart, invocation, endpoint, retErr, version, strictExplicitToken)
	}()
