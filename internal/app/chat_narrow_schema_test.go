...
func TestCrossPlatformCoverageChatNarrowSchemaReplacesRoutineHelp(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.path, func(t *testing.T) {
			...
			assertSchemaConstraintsReferencePublishedParameters(t, testCase.path, parameters, payload["constraints"])
...
func assertSchemaConstraintsReferencePublishedParameters(t *testing.T, cliPath string, parameters map[string]any, rawConstraints any) {
	t.Helper()
	constraints, ok := rawConstraints.(map[string]any)
	if rawConstraints == nil {
		return
	}
	if !ok {
		t.Fatalf("%s constraints = %#v, want object", cliPath, rawConstraints)
	}
	for _, kind := range []string{"mutually_exclusive", "require_one_of", "require_together"} {
		groups, ok := constraints[kind].([]any)
		if !ok && constraints[kind] != nil {
			t.Fatalf("%s constraint %s = %#v, want groups", cliPath, kind, constraints[kind])
		}
		for groupIndex, rawGroup := range groups {
			group, ok := rawGroup.([]any)
			if !ok {
				t.Fatalf("%s constraint %s[%d] = %#v, want names", cliPath, kind, groupIndex, rawGroup)
			}
			for _, rawName := range group {
				name, ok := rawName.(string)
				if !ok {
					t.Fatalf("%s constraint %s[%d] has non-string name %#v", cliPath, kind, groupIndex, rawName)
				}
				if _, published := parameters[name]; !published {
					t.Errorf("%s constraint %s[%d] references unpublished parameter %q", cliPath, kind, groupIndex, name)
				}
			}
		}
	}
