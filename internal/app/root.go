...
var (
	...
	rootAuthLoadTokenData           = func(configDir string) (*authpkg.TokenData, error) {
		return authpkg.LoadTokenDataForProfileReadOnly(configDir, authpkg.RuntimeProfile())
	}
)
...
func loadPlugins(engine *pipeline.Engine, _ executor.Runner) []*cobra.Command {
	...
	hasExplicitToken := preparseExplicitTokenFlag(os.Args[1:])
