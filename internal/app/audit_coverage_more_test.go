...
func TestCrossPlatformCoverageAuditRuntimeCoverage(t *testing.T) {
	...
	emitAudit(nil, "nil", time.Now(), invocation, "https://example.com?token=secret", nil, "test", false)
	emitAudit(audit.NopSink{}, "nop", time.Now(), invocation, "", nil, "test", false)
	recording := &auditCoverageSink{}
	emitAudit(recording, "ok", time.Now(), invocation, "https://example.com?token=secret", nil, "test", false)
	...
	emitAudit(recording, "typed", time.Now(), invocation, "", typed, "test", false)
	if recording.events[1].ErrReason != "expired" {
		t.Fatalf("typed audit event = %#v", recording.events[1])
	}
	recording.emitErr = errors.New("emit")
	emitAudit(recording, "failed", time.Now(), invocation, "", errors.New("plain"), "test", false)
