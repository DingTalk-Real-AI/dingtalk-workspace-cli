...
func emitAudit(sink audit.Sink, execID string, invokeStart time.Time, invocation executor.Invocation, endpoint string, retErr error, cliVersion string) {
	emitAuditWithActorPolicy(sink, execID, invokeStart, invocation, endpoint, retErr, cliVersion, false)
}

func emitAuditWithActorPolicy(sink audit.Sink, execID string, invokeStart time.Time, invocation executor.Invocation, endpoint string, retErr error, cliVersion string, strictExplicitToken bool) {
	if sink == nil {
		return
	}
