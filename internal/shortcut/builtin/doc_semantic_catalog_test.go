...
func TestCrossPlatformCoverageDocSemanticCatalogExactlyCoversRegisteredSurface(t *testing.T) {
	...
	if len(registered) != 49 || len(source.Shortcuts) != 49 {
		t.Fatalf("registered/catalog = %d/%d, want 49/49", len(registered), len(source.Shortcuts))
	}
	...
	if public != 47 || hidden != 2 {
		t.Fatalf("public/hidden = %d/%d, want 47/2", public, hidden)
	}
