...
func TestCrossPlatformCoverageDocFinalExecutionFailureBranches(t *testing.T) {
	...
	t.Run("empty download", func(t *testing.T) {
		...
		_ = runDocCoverage(t, MediaDownload, &docCoverageCaller{responses: map[string][]map[string]any{}}, "--yes", "--node", "n", "--resource-id", "ca246787-99c8-4b8e-9d8f-3f6a2b1c0d4e", "--output", "out")
	})
}
