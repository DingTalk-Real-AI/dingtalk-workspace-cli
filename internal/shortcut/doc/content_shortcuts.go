...
func executePlainTextReplace(rt *shortcut.RuntimeContext) error {
	...
	return rt.CallMCP("update_document_block", map[string]any{"nodeId": rt.Str("node"), "blockId": matches[0].blockID, "element": map[string]any{"blockType": "paragraph", "paragraph": map[string]any{"text": updated}}})
}

func executeBlockCopy(rt *shortcut.RuntimeContext) error {
	data, err := rt.CallMCPData(productDoc, "list_document_blocks", map[string]any{"nodeId": rt.Str("node"), "blockId": rt.Str("block-id"), "format": "element"})
	if err != nil {
		return err
	}
	...
	stripBlockIDs(block)
	return rt.CallMCP("insert_document_block", map[string]any{"nodeId": rt.Str("node"), "referenceBlockId": rt.Str("after-block-id"), "where": "after", "element": block})
}
