...
var Update = shortcut.Shortcut{
	...
	Flags: []shortcut.Flag{
		...
		{Name: "content", Type: shortcut.FlagString, Desc: docRequiredContentInputDescription, RequiredWhen: "--command=append|overwrite|block_insert_after|block_replace"},
		{Name: "doc-format", Type: shortcut.FlagString, Default: "markdown", Desc: "内容格式", Enum: []string{"markdown", "jsonml"}},
		...
		{Name: "after-block-id", Type: shortcut.FlagString, Desc: "插入位置参考 block ID", RequiredWhen: "--command=block_insert_after|block_copy_insert_after"},
...
var Import = shortcut.Shortcut{
	...
	Intent:      "当用户要把工作目录内的 doc/docx/xls/xlsx/md/txt/xmind/mark 相对路径文件导入为钉钉在线对象，并可指定目标文件夹或知识库时使用；CLI 负责上传与转换，不先用本地库解析。",
	Risk:        shortcut.RiskWrite,
	...
	Contract: docContract("+import", "上传本地文件并等待转换成在线文档对象",
		"当用户要把工作目录内的 doc/docx/xls/xlsx/md/txt/xmind/mark 相对路径文件导入为钉钉在线对象，并可指定目标文件夹或知识库时使用；CLI 负责上传与转换，不先用本地库解析。",
...
func executeUpdate(rt *shortcut.RuntimeContext) error {
	...
	case "str_replace":
		return executePlainTextReplace(rt)
	case "block_copy_insert_after":
		return executeBlockCopy(rt)
	default:
...
func executePlainTextReplace(rt *shortcut.RuntimeContext) error {
	nodeID := rt.Str("node")
...
func executeBlockCopy(rt *shortcut.RuntimeContext) error {
	nodeID := rt.Str("node")
...
func executeExportGet(rt *shortcut.RuntimeContext) error {
	...
	result, err := docDownload(rt.Command().Context(), downloadURL, localio.DownloadOptions{
		BaseDir: cwd, Output: rt.Str("output"), PreferredName: "document", Overwrite: rt.Bool("overwrite"),
	})
	if err != nil {
		return docExportRecoveryError("doc_export_download_failed", "download", fmt.Sprintf("导出任务已完成但安全下载失败（output=%s）", rt.Str("output")), jobID, err)
	}
