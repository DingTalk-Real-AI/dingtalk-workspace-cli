...
func TestCrossPlatformCoverageRecordBatchPureHelpers(t *testing.T) {
	...
	if recordCellsMayNeedFieldTypes(map[string]any{}, map[string]any{"f": "value"}) {
		t.Fatal("recordCellsMayNeedFieldTypes accepted a record without cells")
	}
	...
	if _, ok := recordNumericValue(strings.Repeat("1", 129)); ok {
		t.Fatal("recordNumericValue accepted an overlong numeric string")
	}
	if recordDateValueEqual(1, "2026-01-01") {
		t.Fatal("recordDateValueEqual accepted a non-string read-back")
	}
	if scalarProjectionShapes(1, "value") {
		t.Fatal("scalarProjectionShapes accepted a non-string read-back")
	}
}

func TestCrossPlatformCoverageRecordReadbackPendingAndWaitBranches(t *testing.T) {
...
func TestCrossPlatformCoverageRecordUpsertPendingMismatchRetryBranches(t *testing.T) {
	disableRecordReadbackWait(t)
...
func TestResponseCursorFiltersErrorGuardPrefix(t *testing.T) {
	if got := responseCursor(map[string]any{"nextCursor": "error-v1:NON_RESUMABLE_ERROR_CURSOR"}); got != "" {
		t.Fatalf("error-v1: guard cursor must be filtered, got %q", got)
	}
	if got := responseCursor(map[string]any{"cursor": "error-v1:NON_RESUMABLE_ERROR_CURSOR"}); got != "" {
		t.Fatalf("error-v1: guard cursor in cursor key must be filtered, got %q", got)
	}
	if got := responseCursor(map[string]any{"next_cursor": "  error-v1:NON_RESUMABLE_ERROR_CURSOR  "}); got != "" {
		t.Fatalf("error-v1: with whitespace must be filtered, got %q", got)
	}
	if got := responseCursor(map[string]any{"data": map[string]any{"nextCursor": "error-v1:GUARD"}}); got != "" {
		t.Fatalf("nested error-v1: must be filtered, got %q", got)
	}
	if got := responseCursor(map[string]any{"nextCursor": "valid-cursor-abc"}); got != "valid-cursor-abc" {
		t.Fatalf("normal cursor must pass through, got %q", got)
	}
}

func TestResponseHasMoreIgnoresErrorGuardCursor(t *testing.T) {
	if responseHasMore(map[string]any{"nextCursor": "error-v1:NON_RESUMABLE_ERROR_CURSOR"}) {
		t.Fatal("error-v1: guard cursor must not trigger hasMore")
	}
	if responseHasMore(map[string]any{"cursor": "error-v1:NON_RESUMABLE_ERROR_CURSOR"}) {
		t.Fatal("error-v1: in cursor key must not trigger hasMore")
	}
	if !responseHasMore(map[string]any{"nextCursor": "error-v1:NON_RESUMABLE_ERROR_CURSOR", "cursor": "valid-cursor"}) {
		t.Fatal("a valid cursor alongside a guard cursor must still trigger hasMore")
	}
	if responseHasMore(map[string]any{"data": map[string]any{"nextCursor": "error-v1:GUARD"}}) {
