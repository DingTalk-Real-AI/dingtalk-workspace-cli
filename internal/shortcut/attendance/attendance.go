...
var CalculateApproveDuration = shortcut.Shortcut{
	...
	Flags: []shortcut.Flag{
		{Name: "biz-type", Type: shortcut.FlagInt, Desc: "审批业务类型，--biz-type 必须在 1 到 8 之间：1-加班，2-普通审批，3-请假，4-补卡，5-外出，6-换班，7-考勤证明，8-单次外出", Required: true},
		{Name: "approve-biz-type", Type: shortcut.FlagString, Desc: "普通审批场景的原始审批子类型，用于区分外出、出差等；不确定时不传"},
		{Name: "duration-mode", Type: shortcut.FlagInt, Desc: "时长模式，--duration-mode 必须在 1 到 5 之间：1-天，2-半天，3-小时，4-半小时，5-分钟", Required: true},
		{Name: "start", Type: shortcut.FlagString, Desc: "开始时间，--start 必须为 YYYY-MM-DD 或 yyyy-MM-dd HH:mm:ss", Required: true},
		{Name: "end", Type: shortcut.FlagString, Desc: "结束时间，--end 必须为 YYYY-MM-DD 或 yyyy-MM-dd HH:mm:ss，且不得早于 --start", Required: true},
		{Name: "half-start", Type: shortcut.FlagString, Enum: []string{"AM", "PM"}, RequiredWhen: "duration-mode=2", Desc: "半天模式开始时段：AM 或 PM；半天模式必须同时提供 --half-start 和 --half-end"},
		{Name: "half-end", Type: shortcut.FlagString, Enum: []string{"AM", "PM"}, RequiredWhen: "duration-mode=2", Desc: "半天模式结束时段：AM 或 PM；半天模式必须同时提供 --half-start 和 --half-end"},
	...
	Constraints: []shortcut.Constraint{
		...
		{Kind: shortcut.ConstraintCustom, Flags: []string{"duration-mode"}, Description: "--duration-mode 必须在 1 到 5 之间"},
		{Kind: shortcut.ConstraintCustom, Flags: []string{"half-start", "half-end"}, Description: "半天模式必须同时提供 --half-start 和 --half-end"},
		{Kind: shortcut.ConstraintCustom, Flags: []string{"start"}, Description: "--start 必须为 YYYY-MM-DD 或 yyyy-MM-dd HH:mm:ss"},
		{Kind: shortcut.ConstraintCustom, Flags: []string{"end"}, Description: "--end 必须为 YYYY-MM-DD 或 yyyy-MM-dd HH:mm:ss，且不得早于 --start"},
		{Kind: shortcut.ConstraintCustom, Flags: []string{"duration-in-hour", "duration-in-day"}, Description: "--duration-in-hour/--duration-in-day 互斥，必须为大于 0 的数字且与 --duration-mode 单位匹配"},
		{Kind: shortcut.ConstraintCustom, Flags: []string{"detail-list"}, Description: "--detail-list 必须为 JSON 数组，每项含 workDate（yyyy-MM-dd HH:mm:ss 或 13 位毫秒时间戳整数）与大于 0 的 durationInHour 或 durationInDay（互斥且与单位匹配）"},
		{Kind: shortcut.ConstraintCustom, Flags: []string{"modified-date"}, Description: "--modified-date 格式为 yyyy-MM-dd HH:mm:ss"},
	},
	Validate: func(rt *shortcut.RuntimeContext) error {
...
var CheckCompanionSchedules = shortcut.Shortcut{
	...
	Flags: []shortcut.Flag{
		{Name: "approve-type", Type: shortcut.FlagInt, Desc: "审批类型，--approve-type 只能为 1（出差）或 2（外出）", Required: true},
		{Name: "starts", Type: shortcut.FlagStringSlice, Desc: "申请时间段开始时间列表，不能为空，YYYY-MM-DD 或 yyyy-MM-dd HH:mm:ss；与 --ends 按位置一一对应", Required: true},
		{Name: "ends", Type: shortcut.FlagStringSlice, Desc: "申请时间段结束时间列表，不能为空，格式同 --starts；与 --starts 数量必须一致", Required: true},
...
var GetComplexOvertimeSetting = shortcut.Shortcut{
	...
	Flags: []shortcut.Flag{
		...
		{Name: "work-date", Type: shortcut.FlagString, Desc: "规则生效日期，--work-date 如传入，必须为 YYYY-MM-DD 或 yyyy-MM-dd HH:mm:ss；不传时使用服务端当前时间"},
	},
	Constraints: []shortcut.Constraint{
