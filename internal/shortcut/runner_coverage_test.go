...
func TestCrossPlatformCoverageRuntimeContextForTest(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), runtimeContextKey{}, "runtime-context")
	cmd.SetContext(ctx)
	...
	if rt.Context().Value(runtimeContextKey{}) != "runtime-context" {
		t.Fatal("RuntimeContext.Context did not expose the command context")
	}
}
