...
func TestWaitTimeoutFlagDefaultsComeFromDeclaration(t *testing.T) {
	...
	// An explicit non-positive value remains visible to validation.
...
func TestWaitTimeoutDurationRejectsOverflowingSeconds(t *testing.T) {
	...
	for _, secs := range []int64{0, -5} {
		if got, err := waitTimeoutDuration(secs); err == nil || got != 0 || !strings.Contains(err.Error(), "必须为正数") {
			t.Fatalf("duration(%d)=%d/%v, want positive-value validation", secs, got, err)
		}
	}
}
...
func TestResultInvokeWaitTimeoutOverflowIsValidationError(t *testing.T) {
	...
	spec.ResultInvoke = func(*Ctx, map[string]any) (output.CommandResult, error) {
		invoked = true
		return output.Pending(map[string]any{"id": "job-1"}, &output.OperationInfo{ID: "job-1", State: "NEW"}), nil
	}
	cmd := New(spec)
	...
	if invoked {
		t.Fatal("overflowing --wait-timeout must be rejected before ResultInvoke")
	}
	if polled {
...
func TestResultInvokeWaitTimeoutNonPositiveIsValidationError(t *testing.T) {
	for _, raw := range []string{"0", "-1"} {
		t.Run(raw, func(t *testing.T) {
			...
			cmd := New(spec)
			cmd.SetArgs([]string{"--wait", "--wait-timeout", raw})
			ctx, _ := output.WithResultStore(context.Background())
			cmd.SetContext(ctx)
			err := cmd.Execute()
			if err == nil || !strings.Contains(err.Error(), "必须为正数") {
				t.Fatalf("err=%v, want positive-value validation", err)
			}
			if invoked {
				t.Fatal("non-positive --wait-timeout must be rejected before ResultInvoke")
			}
			if polled {
				t.Fatal("non-positive --wait-timeout must not start the wait loop")
			}
		})
	}
}
