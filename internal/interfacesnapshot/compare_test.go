...
func TestCrossPlatformCoverageCompareAllowsChatIDCanonicalHiddenAliasMigration(t *testing.T) {
	base := testSnapshot(
		testCommand("dws"),
		testCommand("dws chat message edit",
			testFlag("group", "string", false),
			testFlag("msg-id", "string", false),
		),
	)
	hiddenGroup := testFlag("group", "string", false)
	hiddenGroup.Hidden = true
	hiddenMsgID := testFlag("msg-id", "string", false)
	hiddenMsgID.Hidden = true
	current := testSnapshot(
		testCommand("dws"),
		testCommand("dws chat message edit",
			testFlag("conversation-id", "string", true),
			hiddenGroup,
			testFlag("message-id", "string", true),
			hiddenMsgID,
		),
	)
	...
	comparison := Compare(current, base, "base")
	if !comparison.Compatible {
		t.Fatalf("chat ID canonical migration was blocked: %#v", comparison.Blocking)
	}
}

func TestCrossPlatformCoverageCompareAllowsChatIDLegacyFlagRequiredMigration(t *testing.T) {
	base := testSnapshot(
		testCommand("dws"),
		testCommand("dws chat message edit", testFlag("group", "string", false)),
	)
	current := testSnapshot(
		testCommand("dws"),
		testCommand("dws chat message edit",
			testFlag("conversation-id", "string", false),
			testFlag("group", "string", true),
		),
	)

	comparison := Compare(current, base, "base")
	if !comparison.Compatible {
		t.Fatalf("chat ID legacy required migration was blocked: %#v", comparison.Blocking)
	}
}

func TestCrossPlatformCoverageChatIDMigrationGuardBranches(t *testing.T) {
	historical := map[string]Flag{
		"group": testFlag("group", "string", false),
	}
	current := map[string]Flag{
		"conversation-id": testFlag("conversation-id", "string", false),
		"group":           testFlag("group", "string", false),
	}
	hiddenGroupCurrent := map[string]Flag{
		"conversation-id": testFlag("conversation-id", "string", false),
		"group": func() Flag {
			flag := testFlag("group", "string", false)
			flag.Hidden = true
			return flag
		}(),
	}
	if !allowedChatIDFlagMigration("dws chat message edit", "conversation-id", historical, hiddenGroupCurrent) {
		t.Fatal("canonical migration with a hidden historical alias should be allowed")
	}
	if allowedChatIDFlagMigration("dws chat message edit", "query", historical, current) {
		t.Fatal("non-ID flags must not be allowed")
	}
	if allowedChatIDFlagMigration("dws chat message edit", "group", historical, map[string]Flag{}) {
		t.Fatal("missing canonical flag must not be allowed")
	}
	if allowedChatIDFlagMigration("dws chat message edit", "conversation-id", historical, current) {
		t.Fatal("canonical migration without hidden historical alias must not be allowed")
	}
	if allowedChatIDRequiredAddition("dws chat message edit", "group", historical, current) {
		t.Fatal("required additions are allowed only for canonical flag names")
	}
	hiddenCanonical := map[string]Flag{
		"conversation-id": func() Flag {
			flag := testFlag("conversation-id", "string", false)
			flag.Hidden = true
			return flag
		}(),
		"group": hiddenGroupCurrent["group"],
	}
	if allowedChatIDRequiredAddition("dws chat message edit", "conversation-id", historical, hiddenCanonical) {
		t.Fatal("required additions with hidden canonical flag must not be allowed")
	}
	if allowedChatIDRequiredAddition("dws chat message edit", "conversation-id", nil, hiddenGroupCurrent) {
		t.Fatal("required additions without historical aliases must not be allowed")
	}
	...
	if got := canonicalChatIDFlag("userId"); got != "user-id" {
		t.Fatalf("canonical userId = %q, want user-id", got)
	}
	for _, canonical := range []string{"message-id", "user-id", "unknown"} {
		_ = chatIDFlagAliases(canonical)
	}
}

func TestCrossPlatformCoverageCompareBlocksNonChatHiddenAliasMigration(t *testing.T) {
	base := testSnapshot(
		testCommand("dws"),
		testCommand("dws drive copy", testFlag("group", "string", false)),
	)
	hiddenGroup := testFlag("group", "string", false)
	hiddenGroup.Hidden = true
	current := testSnapshot(
		testCommand("dws"),
		testCommand("dws drive copy",
			testFlag("conversation-id", "string", true),
			hiddenGroup,
		),
	)
	...
	comparison := Compare(current, base, "base")
	if comparison.Compatible || !hasFlagChange(comparison.Blocking, "flag_became_hidden", "dws drive copy", "group") {
		t.Fatalf("non-chat ID migration was not blocked: %#v", comparison.Blocking)
	}
}
