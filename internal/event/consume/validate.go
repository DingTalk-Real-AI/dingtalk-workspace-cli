...
var (
	...
	ErrForceRequiresForeground = &ValidationError{
		Msg: "--force is only meaningful with --foreground (in daemon mode it would produce multiple bus instances; cloud events would be randomly split across connections). To restart the bus: preview dws event stop --all --dry-run, confirm with --yes, then run dws event consume",
	}
