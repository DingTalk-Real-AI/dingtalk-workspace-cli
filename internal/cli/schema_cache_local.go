...
func clearIdentityFilesUnderRoot(root *os.Root, relative string) {
	if dir, err := root.Open(relative); err == nil {
		defer dir.Close()
		if entries, err := dir.ReadDir(-1); err == nil {
			removeIdentityEntriesUnderRoot(root, entries, relative)
		}
	}
}

func removeIdentityEntriesUnderRoot(root *os.Root, entries []os.DirEntry, relative string) {
