...
func NewSchemaCommand() *cobra.Command {
	return NewSchemaCommandWithRootProvider(nil)
}

// NewSchemaCommandWithRootProvider renders Schema from a version-stable
// command tree supplied by the owning application. Runtime command execution
// may still merge registry/plugin commands, but those environment-dependent
// trees must not change the release's Agent contract.
func NewSchemaCommandWithRootProvider(rootProvider func() *cobra.Command) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "schema [path]",
...
（parameters 内联 required，键为 CLI flag）：
  ...
  dws schema "dev app"                      # 列出该分组下的子命令`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// `schema list` was used by an earlier schema browser. Keep it as
			// a stable compatibility alias for the root overview, rather than
			// treating "list" as a product path.
			...
			// Release binaries serve the versioned catalog directly. The root
			// provider is retained for generators and focused tests that need to
			// render a candidate Cobra tree before committing a new snapshot.
			if rootProvider == nil && embeddedSchemaCatalogAvailable() {
			...
			schemaRoot := cmd.Root()
			if rootProvider != nil {
				schemaRoot = rootProvider()
				if schemaRoot == nil {
					return apperrors.NewInternal("schema root provider returned nil")
				}
			}
			...
			if len(args) > 0 {
				payload, ok, err := renderHelperSchema(schemaRoot, args[0])
				if err != nil {
					return err
				}
				if ok {
			...
			payload, err := runtimeSchemaPayload(schemaRoot, args)
			if err != nil {
				return err
			}
			...
			if len(args) == 0 {
				if helpers := helperProductSummaries(schemaRoot); len(helpers) > 0 {
