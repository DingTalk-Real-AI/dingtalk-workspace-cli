...
`chat` 有 {len(canonical)} 条 canonical Shortcut：根 Help 展示 {len(featured)} 条 Featured，另 {catalog_count} 条在 Catalog、Schema 和精确 Help；{compatibility_count} 条兼容、{unavailable_count} 条 unavailable 不参与默认选路。
