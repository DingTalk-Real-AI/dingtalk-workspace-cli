...
def main():
    ...
    flags = ["-s", "-w", "-X", f"main.version={args.version}", "-X", f"main.commit={commit}", "-X", f"main.buildTime={build_time}"]
    for field, key in fields.items():
        flags += ["-X", f"github.com/DingTalk-Real-AI/dingtalk-workspace-cli/internal/app.{field}={proof[key]}"]
    run(["go", "build", "-trimpath", "-buildmode=pie", "-ldflags", " ".join(flags), "-o", str(core), "./cmd"])
