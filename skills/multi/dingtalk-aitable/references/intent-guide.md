...
| 已确认是 able 的 AITable 链接，需要读取记录 | AITable | 用 `+url-resolve` 取稳定 ID，再用 `+record-query`；用户原始 `/i/nodes/` URL 先 `drive info`，`extension=dlink` 时按目标 `linkSourceInfo.nodeId` 继续解析，不能把入口 ID 当 baseId |
