...
| `dashboard share update` | 更新仪表盘分享配置 | `--base-id` `--dashboard-id` `--enabled` | `--enabled true` 开启，`--share-type` 用 `ORG`（ORG 回显 `shareType="[1]"`）；`PUBLIC` 常被组织策略禁用，真机实测报 `Illegal argument (600)`，请用 `ORG`。`--enabled false` 关闭 |
...
| `chart share update` | 更新图表分享配置（`--enabled true/false`，开启配 `--share-type`，用 `ORG`；`PUBLIC` 常被组织策略禁用，真机报 `Illegal argument (600)`） | `--base-id` `--dashboard-id` `--chart-id` `--enabled` |
