...
| "这个 alidocs 表格链接帮我看下"（粘贴原始 URL） | 先 probe 节点类型 | `dws drive info --node`；`extension=dlink` 再用 `dws doc info --node <快捷方式nodeId>` 取 `linkSourceInfo`，最后按目标 `extension` 路由 | 直接调 `sheet` | `alidocs/i/nodes/{id}` 可能是文档/快捷方式/axls/able/xlsx 等，禁止凭 URL 猜类型 |
