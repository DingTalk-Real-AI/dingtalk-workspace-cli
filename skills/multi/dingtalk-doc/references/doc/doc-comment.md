...
> **能力边界：** `create-inline` 不支持 @群。不得只在正文中写 `@群名` 冒充真实群 mention。
...
- `--mentioned-open-conversation-id` 接受群 `openConversationId`，可重复传入或逗号分隔；仅全文评论 `create`、`reply`、`update` 支持，`create-inline` 不支持。
- 群名先用 `dws chat search --query "<群名>" --format json` 唯一解析；只在正文写 `@群名` 不会创建真实 mention。
