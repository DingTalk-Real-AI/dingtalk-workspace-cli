...
若请求是“今日日程 + 当前未完成待办 + 最近一条听记摘要”的全部或任意组合，使用根 Skill 中的 `scripts/cross_product_read_summary.py`。该路径已内建产品选择、并发采集、分页和紧凑输出；不要再读取下方产品行动指南或产品 Skill。

## 选择顺序

1. 按下表选择与用户目标最匹配的产品行动指南。
2. 未命中上述紧凑快路径时，显式读取流程涉及的产品 `SKILL.md`，不要预加载无关产品。
...
- 涉及多源采集、批量处理、交叉分析、汇总/归纳或三个以上产品，且未命中紧凑只读快路径：使用上表的完整
