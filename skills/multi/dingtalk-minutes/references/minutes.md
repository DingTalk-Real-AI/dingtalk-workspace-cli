...
> **默认 scope 规则**：`dws minutes list` 后**必须**跟 scope 子命令。若用户未明确指定查询范围，**一律默认补 `all`**（查询所有有权限的听记，覆盖面最广）。
...
| `dws minutes list mine --limit 10` | `--limit` 不存在（其他模块用 `--limit`，minutes 用 `--max`） | `dws minutes list mine --max 10` |
...
| `dws report inbox --format json` | `inbox` 不是 report 的子命令（可用：create/detail/list/sent/stats/template） | `dws report list --start "2026-05-06T00:00:00+08:00" --format json` |
| `dws report list --format json`（不带 --start） | report list **必须**带 `--start` 参数，否则报 "flag --start is required" | `dws report list --start "2026-05-06T00:00:00+08:00" --end "2026-05-13T23:59:59+08:00" --format json` |
...
| "帮我看/总结某次会议"（未给 id） | 先 `list mine/all` | 按标题/时间匹配后再 get |
| "上上周/两周前的会议" | `list mine --start --end` | 需正确计算时间范围 |
...
> - report list sent **必须**带 `--start` 参数（格式 `2026-05-04T00:00:00+08:00`），否则报错
> - `dws report inbox` **不存在**——report 可用子命令为 create/detail/list/sent/stats/template。LLM 高频误猜 `inbox`，需明确跳过
> - report list 不传 `--start` 会直接报错 "flag --start is required"，**不要让用户手动补**——AI 应自动推断最近 7 天范围填入
