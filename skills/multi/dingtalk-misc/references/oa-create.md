...
1. **先查 Schema 再收集表单值：** `form-schema` 后向用户展示需要填写的控件列表，然后一次性收集全部表单值。不要在未拿到 Schema 前就问用户填什么。

2. **流程预测后再选自选审批人：** `forecast-process` 返回流程路径和自选节点后：
