...
| `drive`           | 钉钉云盘：文件列表/元数据/文件夹/上传/下载/导出导入异步任务统一查询 | [drive.md](./references/products/drive.md)                     |
...
用户提到"查导出任务/导入任务状态/已有任务 ID" → `drive task get --type export --id <TASK_ID>`（导入任务改用 `--type import`）
