---
name: dws
description: 管理钉钉产品能力(AI表格/AI搜问/日历/通讯录/群聊与机器人/待办/审批/考勤/日志/DING消息/开放平台文档/开放平台应用/钉钉文档/钉钉云盘/AI听记/邮箱/在线电子表格/知识库等)。当用户需要操作表格数据、管理日程会议、模糊找人/查谁负责某事项、查询通讯录、管理群聊、机器人发消息、创建待办、提交审批、查看考勤、提交日报周报（钉钉日志模版）、读写钉钉文档、上传下载云盘文件、查询听记纪要、收发邮件、读写在线电子表格(axls)、管理钉钉知识库、管理开放平台企业内部应用、应用成员、安全配置或应用权限时使用。
cli_version: ">=1.0.15"
---
...
| `devapp`          | 开放平台应用：企业内部应用查询/详情/创建/修改/删除、成员管理、安全配置、应用权限 APP/SNS 列表与申请取消、凭证读取、机器人创建与配置、版本创建/发布/审批、事件订阅/退订       | [devapp.md](./references/products/devapp.md)                   |
...
- "应用" 是泛词，不默认等于开放平台应用。只有出现"开放平台应用/开发者后台应用/企业内部应用"，
  或伴随 `agentId/clientId/appKey/appSecret/customKey/应用成员/安全配置/IP 白名单/登录重定向/应用权限/权限点/创建机器人/智能体机器人/机器人配置/事件订阅/订阅事件/退订事件/eventCode/应用版本/发布审核`
  等开发者后台标识时，才路由到 `devapp`。
- "工作台应用/app001/appXYZ/钉钉工作台上有哪些应用" → `workbench app`，不要走 `devapp`。
- "开放平台接口文档/API 怎么调用/错误码/字段说明" → `devdoc`，不要走 `devapp`。
- "MCP 服务/connector/创建工具/HSF 映射/上架 MCP" → MCP 平台配置流程，不是 `dws devapp create`。
- 只说"应用"且缺少上下文时，先结合当前对话判断；仍不明确时追问是开放平台企业内部应用、工作台应用、文档能力还是 MCP 工具。
...
用户提到"开放平台应用/开发者后台应用/企业内部应用/应用 agentId/clientId/appKey/appSecret/应用成员/安全配置/IP 白名单/登录重定向/应用权限/权限点/创建机器人/智能体机器人/机器人配置/事件订阅/订阅事件/退订事件/eventCode/应用版本发布/审核" → `devapp`
