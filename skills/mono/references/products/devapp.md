...
3. 写操作统一通过 `--unified-app-id` 定位应用；不要用 name/appKey 做写操作定位。
...
除 `list` 支持 `--name` 搜索外，详情、凭证、网页应用、权限和写操作都使用 `--unified-app-id`。
...
MCP tool: `get_dev_app_credentials`。返回含 `clientSecret/appSecret`，按敏感凭证处理。不能用 `devapp get` 代替；如果 `devapp get` 偶尔返回密钥，也只用于内部判断并脱敏。
...
MCP tools: `get_extension_webapp_config` / `set_extension_webapp_config`
...
MCP tool: `apply_dev_app_permissions`。`requiredApproval=true` 允许申请，写入版本变更。

### 取消权限

```bash
dws devapp permission remove --unified-app-id ID --permissions qyapi_robot_sendmsg --dry-run --format json
dws devapp permission remove --unified-app-id ID --permissions Contact.User.mobile,qyapi_robot_sendmsg --dry-run --format json
```

MCP tool: `remove_dev_app_permissions`。`--permissions` 接收一个或多个 `scopeValue`，多个用逗号分隔；旧 `--permission/--scope` 仅作为兼容别名。
...
```bash
dws devapp security config --unified-app-id UNIFIED_APP_ID --ip-whitelist 192.0.2.10 --redirect-url https://callback.example.invalid/callback --dry-run --format json
```
...
- **两种来源**：① 直接 `--robot-client-id/--robot-client-secret`；② `--unified-app-id`（内部调 `get_dev_app_credentials` 自动取）。⚠️ 来源②**字段名待预发真机验证**，且 clientSecret 一般仅建号时返回一次、该接口未必能返回，取不到时回退手填。
