...
管理钉钉开放平台企业内部应用。用于应用列表查询、应用详情、创建/修改/删除、成员管理、安全配置、凭证读取、权限查询/申请/取消，以及事件订阅和版本发布的目标指令设计。
...
P0 可用目标是应用 CRUD、网页应用配置、成员管理、安全配置和权限；凭证、事件、版本发布依赖后端 MCP tool 是否已发布。
...
```text
list_open_dev_app_members
add_open_dev_app_members
remove_open_dev_app_members
```
...
| 删除应用 | `devapp delete` |
| 查看应用成员、开发者成员 | `devapp member list` |
| 添加应用成员、开发者成员 | `devapp member add --dry-run`，确认后 `--yes` |
| 移除应用成员、开发者成员 | `devapp member remove --dry-run`，确认后 `--yes` |
| 配置 IP 白名单、登录重定向 URL、端内免登 URL | `devapp security config --dry-run`，确认后 `--yes` |
