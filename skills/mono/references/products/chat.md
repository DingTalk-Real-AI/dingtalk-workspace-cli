...
注意:
  ...
  - 话题圈使用 `openTopicId`，圈内一条话题使用 `openConvThreadId`。浏览主话题使用 `dws chat topic list`；读取回复串使用 `dws chat topic list-replies --open-topic-id <openTopicId> --open-conv-thread-id <openConvThreadId>`。
```
...
分页查询指定话题的回复，每次返回一页。`openTopicId` 指定话题圈，`openConvThreadId` 指定圈内一条话题；需要自动读取全部页面时使用现有的 `dws chat +thread-replies --page-all`。
```
Usage:
  dws chat topic list-replies [flags]
Example:
  dws chat topic list-replies --open-topic-id <openTopicId> --open-conv-thread-id <openConvThreadId>
  dws chat topic list-replies --open-topic-id <openTopicId> --open-conv-thread-id <openConvThreadId> --time "2025-03-01 00:00:00" --limit 20
Flags:
      --open-topic-id string        话题圈 openTopicId (必填)
      --open-conv-thread-id string  话题 openConvThreadId (必填)
...
```
Usage:
  dws chat topic forward [flags]
Example:
  dws chat topic forward --src-msg-id <openMessageId> --src-open-topic-id <openTopicId> --src-open-conv-thread-id <openConvThreadId> --dest-open-conversation-id <openConversationId>
Flags:
      --src-msg-id string                 源话题主消息 messageId (必填)
      --src-open-topic-id string          源话题圈 openTopicId (必填)
      --src-open-conv-thread-id string    源话题 openConvThreadId (必填)
      --dest-open-conversation-id string  目标会话 openConversationId (必填)
```
...
用户说"群里某条话题的回复" → `chat topic list-replies --open-topic-id <openTopicId> --open-conv-thread-id <openConvThreadId>`
...
用户说"回复话题" → `chat topic reply`
用户说"查看话题回复/拉取话题回复" → `chat topic list-replies`
...
用户说"转发话题/转发话题消息" → `chat topic forward`
...
- `chat topic list-replies` — 使用 `openTopicId/openConvThreadId` 拉取话题回复；只有需要按主消息自动解析、全量翻页、排序或下载资源时才使用现有的 `chat +thread-replies`
