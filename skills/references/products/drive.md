...
用户说"文件详情/文件信息" → `info`
用户说"下载文件" → `download` 拿临时 URL，再由 Agent 自行发起 HTTP GET
用户说"新建文件夹/创建目录" → `mkdir`
用户说"上传文件/传文件到钉盘" → 三步走: `upload-info` → HTTP PUT 到预签名 URL → `commit`（开源版**没有**一条聚合命令，必须完整走完三步）
用户说"复制文件/移动文件/搬到/移到" → 使用 `dws doc copy` / `dws doc move`（详见下方「复制/移动钉盘文件」工作流）
用户说"删除文件/删除文件夹/移到回收站" → 暂未在开源版 CLI 暴露顶层命令，请在钉钉客户端操作（或参考 capability-limits.md）
