...
**工具调用**（可选）: 对钉钉文档链接使用 `dws doc get --url <链接>` 或 `dws doc search --query "<文档标题>"` 获取标题摘要。
...
**工具调用**: `dws doc update --node <nodeId> --mode overwrite --markdown "<结构化提取的完整摘要内容>" --format json`

**参数说明**:
- `--node`: 上一步创建的文档 ID
- `--mode overwrite`: 覆盖模式，清空原文档后写入新内容
- `--markdown`: 结构化提取的完整 Markdown 摘要内容

**返回示例**:
```json
{
  "logId": "2104a64c17741877111978400e0a8a",
...
**注意事项**:
- Markdown 内容较长时，确保 shell 命令正确转义
- 如遇认证问题，在父进程验证 token 有效性
- 成功后从返回结果中提取 `docUrl` 返回给用户
...
📊 文档内容概览：
- 核心议题：{topic_count} 个
- 关键决策：{decision_count} 项
- 行动项：{action_count} 项待办
