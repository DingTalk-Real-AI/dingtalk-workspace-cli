...
- Prompt: 监听我收到的所有单聊消息
- Expected: `dws event consume user_im_message_receive_o2o_all -f ndjson`
...
- Prompt: 监听我所在的所有群消息
- Expected: `dws event consume user_im_message_receive_group_all -f ndjson`
...
- Expected: `dws event consume user_im_group_updated --group cid123 -f ndjson`
- Flags: `--group` = `cid123`
...
- Expected: `dws event consume user_im_group_member_added --group cid123 -f ndjson`
- Flags: `--group` = `cid123`
- Contract: 只读取顶层公共字段和实际 `payload`，不得猜测成员或操作者字段
...
- Expected: `dws event consume user_im_group_member_exited --group cid123 -f ndjson`
- Flags: `--group` = `cid123`
- Contract: 只读取顶层公共字段和实际 `payload`，不得猜测成员或操作者字段
...
- Expected: `dws event consume user_im_group_disbanded --group cid-test-disbanded -f ndjson`
...
- Expected: `dws event consume user_im_message_receive_o2o user_im_message_read_o2o user_im_message_recall_o2o --user test-user-001 -f ndjson`
- Flags: `--user` = `test-user-001`
...
- Expected: `dws event consume user_im_message_receive_group user_im_group_updated user_im_group_disbanded --group cid123 -f ndjson`
- Flags: `--group` = `cid123`
