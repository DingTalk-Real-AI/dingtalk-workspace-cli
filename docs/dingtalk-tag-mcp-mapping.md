...
- CLI 的 `capability mcp create|update|delete|list|query` 均必填 `--agent-uuid`。
- `create` 和带配置的 `update` 在 CLI 内部先调用 `check_mcp`；`check_mcp` 不作为 CLI 子命令暴露。
- MCP 平台配置不在 Git 仓库内；下表是需要平台管理员发布的映射，不代表已经生效。
- OpenAPI 的 check/create/query/list/update/delete 六个 MCP 方法均必填 agentUuid。
...
| delete_mcp | deleteMcp / McpDeleteRequest | 工具.agentUuid → 请求.agentUuid |

六个工具的 inputSchema 根节点增加 agentUuid，type=string，required，
...
## 创建 / 更新的内部校验：不要多包一层 config

`create_mcp`、`update_mcp` 和内部 `check_mcp` 工具的配置字段位于根节点，HSF 的配置字段位于 config 对象内。
...
query_mcp/delete_mcp 的 mcpId、list_mcps 的 keywords/page/pageSize 保持同名映射。
update_mcp 按现有服务端更新契约映射 mcpId、enabled、config，保留未传与 false/空容器的区别，
不要用默认值填充未传字段。delete_mcp 删除资源并由服务端清理 draft 挂载，不自动发布。
...
| 工具 | 建议标题 | 工具说明 |
| --- | --- | --- |
| create_mcp | 创建 MCP 并挂载到数字员工草稿 | 在必填 agentUuid 对应员工域创建并查询 MCP，自动将原 mcpId 追加到 draft.selectedSkills，保存并回读确认后返回成功。保留已有 Skill/MCP 和其他选择，不克隆、不自动发布。失败时检查 errorMsg 的 stage 和 data/错误中的已创建 mcpId，先查询资源和草稿，不要重复创建。同一员工的创建、更新、删除和发布应串行执行。 |
| check_mcp | 校验 MCP 配置 | 在目标 agentUuid 员工域校验 MCP 配置及工具解析结果，不保存资源、不写草稿、不发布。CLI 仅在 create 和带配置的 update 内部调用，不对用户暴露独立命令。 |
...
| delete_mcp | 删除数字员工 MCP | 按 agentUuid + mcpId 删除该员工域 MCP，并清理 draft.selectedSkills 中的挂载；不改变其他 Skill/MCP，不自动发布。 |
...
失败 envelope 保持 `success/errorCode/errorMsg/data`：`stage=mount_draft` 时 data 保留 mcpId 和安全详情；`stage=query_created_mcp` 时 data 可以为空，原 ID 在 errorMsg。若缺少 ID 或请求超时，先核查资源和草稿，不盲目重试；挂载恢复由服务端运维链路处理，不要求普通用户通过 `manage save-draft` 拼装完整数组。不要把 `success=false` 的已创建资源当成“什么都没写”。
...
5. create 和带配置的 update 在内部 check 通过后才写入；check 不写资源/草稿，CLI 不暴露独立 check 命令。update 保持选择，移除后 update 也不重新挂载。
6. delete 删除资源并清理 draft 挂载，不改变其他选择；published 不因 delete 自动改变。
7. 覆盖资源创建后 query 失败、挂载失败和挂载回读失败：返回失败阶段及原 ID，可查后恢复、不重复创建；覆盖首次草稿、已有资源及 local_agent 模式保留。
8. 校验未授权员工被拒绝、dry-run/错误不泄露凭据，且旧公共资源克隆及显式清空选择语义不变。资源已创建和草稿已挂载分别记录证据，不用 list 成功代替草稿核验。
