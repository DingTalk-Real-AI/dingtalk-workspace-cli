...
| DSH | 现有宿主曾可查询；后续宿主退出，修复后查询保守返回不可达 | 宿主本地运行、控制、路由、渲染、审计、会话、配置相关测试 35/35 通过；DWS 身份隔离/就绪判断通过 | 未验收 |
...
重点覆盖未连接、connecting、connected、reconnecting、idle、旧版未观测状态、IPC-only、系统 ping/disconnect、列表分页及时间戳类型。真实消息验收仍需使用升级后的员工 Bus，核对客户端消息、Source/Hub/consumer 计数、Agent 回合与实际回复。
