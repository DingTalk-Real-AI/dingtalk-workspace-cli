...
门槛版本说明：主 RFC 于 2026-09-06 选定受限热路径 runtime，并在 §8.4 废止同包 core 的 5% release gate。本文既有报告按其原始规则保留，不能将旧 `passed/gates` 当成新设计验收；当前代码差距见[主 RFC §10](rfc-schema-runtime-cache.md)。新门槛须在代码对齐后复验。
