...
5. 安装器在 darwin/linux 的 amd64/arm64 上尝试写入共享 cache（Linux `/var/cache/dws`，macOS `/Library/Caches/dws`）；仅当 Meta/Registry/Payloads 与 identity sidecar 均已写出才宣称成功。Windows 及其他 os/arch 跳过（后端按 build tag 编译掉）。
