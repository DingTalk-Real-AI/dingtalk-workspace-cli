...
| **Wait** | `mode`（今日仅 `poll`；event/auto 未实现，声明即校验失败）, `poll_command`, `status_query`, `terminal`（状态→success/failure）, `pending_values`, `default_timeout_secs` | **声明**（`ContractDecl.Wait` 正能力声明，且必须搭配 ResultInvoke dispatcher + `WaitPoll` hook，构造期三方配对校验） | 声明后注册 `--wait`/`--wait-timeout`（框架 flag，不进 toolArgs）；Schema 投影 `wait` 键 | 否；未声明命令传 `--wait` = unknown flag。终态失败经统一信封 `error.type: "wait"`（rc=8），超时保持 pending + `meta.operation.timed_out`（rc=0）；轮询间/轮询中超时一律按 pending 关闭 |
