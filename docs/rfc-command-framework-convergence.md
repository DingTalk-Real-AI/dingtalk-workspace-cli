...
| **Wait** | `mode`（poll/event/auto）, `poll_command`, `status_query`, `terminal`（状态→success/failure）, `pending_values`, `event_key`/`match_field`, `default_timeout_secs` | **声明**（`ContractDecl.Wait` 正能力声明） | `Contract.Wait` + `WaitPoll` hook 配对（构造期校验）；声明后注册 `--wait`/`--wait-timeout`（框架 flag，不进 toolArgs）；Schema 投影 `wait` 键 | 否；未声明命令传 `--wait` = unknown flag。终态失败经统一信封 `error.type: "wait"`（rc=8），超时保持 pending + `meta.operation.timed_out`（rc=0） |
