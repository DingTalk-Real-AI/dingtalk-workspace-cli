...
| 2 | 中文与算法 | BM25 不是已证明最优；TF-IDF/BM25+ 差异区间跨 0；中文短 query 是残余盲区 | Go 保留 BM25 control、TF-IDF shadow；Dense 外置；短 query 必须进入独立 slice，不在同源长句上盲调 |
...
| 5 | Search→Inspect/Provider | Inspect 与 RFC envelope 不一致；Provider 无版本/强 timeout/panic 隔离 | 实现双 hash `schema-inspect.v1`；Provider Catalog handshake、deadline、panic recover 和 one-slot bulkhead |
| 6 | 安全与预算 | Bidi 可回显；8 KiB 未计 newline；摘要静默截断 | RejectControlChars；按最终 wire bytes 计预算；`truncated_fields` 可观察 |
| 7 | 测试/race | 手工恢复全局 seam 违反 policy；alias/NFKC 未穷举；异常 Provider 覆盖不足 | 使用 `testseam.Swap`；增加 alias/NFKC trust；panic/late/ignore-context normal+race 回归 |
...
- Provider：timeout、error、panic、late success、忽略 context、duplicate、unknown、stale Catalog、filtered candidate。
- Budget：query、aggregate subquery、request、summary、完整 response 与 encoder newline 边界。
- Security：危险 Unicode、Provider 错误脱敏、ToolReference 不携带参数 Schema/执行载荷。
