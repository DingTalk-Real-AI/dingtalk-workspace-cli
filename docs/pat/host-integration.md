...
|---|---|---|
| `dws pat chmod --session-id` fallback | `DWS_SESSION_ID` | `REWIND_SESSION_ID` |
| `dws pat chmod --agentCode` fallback | `DINGTALK_DWS_AGENTCODE` | （无） |
...
def run_with_pat_recovery(argv, env, agent_code, session_id, max_recursion=3):
    ...
    # A-core scope: 仅实现 LOW/MEDIUM 风险续授权。
    # HIGH risk (PAT_HIGH_RISK_NO_PERMISSION) 的异步审批通道、
    # PAT_SCOPE_AUTH_REQUIRED 的 scope 补授权（dws auth login --scope）、
    # 以及 dws pat apply/status/scopes 的主动申请 / 状态轮询 / 枚举
    # 由后续 PR A-ext 扩展；此处仅按 JSON 回显，交由宿主决定兜底策略。
...
> **A-core scope**：本 PR 仅把 `PAT_HIGH_RISK_NO_PERMISSION` classifier + stderr JSON 回显落地；
> 完整的异步审批通道 / `authRequestId` 注册表 / `dws pat status` 状态轮询属于后续 PR A-ext。
>
> 宿主在 A-core 环境下最低限度的处理：捕获 `code == PAT_HIGH_RISK_NO_PERMISSION` →
> 透传 `data` 字段给上层 UI → 提示用户走人工 / 带外审批。不要尝试 `dws pat chmod --grant-type permanent` 绕过。
...
6. **不要**依赖 `dws pat apply` / `dws pat status` / `dws pat scopes` / `dws pat callback` / `dws pat approve` 等命令面；A-core 仅提供 `dws pat chmod`，其余命令在后续扩展 PR（A-ext）中发布。
