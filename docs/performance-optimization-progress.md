...
| Schema verified cache | 已实现，待两平台复验 | candidate builder 和 release linker contract 绑定同一 identity |
...
| 固定 main 端到端 | 本地通过，待 clean-head CI | 基准固定为 `6f71222b...`；12/12 入口 gate 和 44 场景五维矩阵通过 |
| Lark/GWS 新 head 对比 | 本地已运行 | wall 延迟五个可比场景快于 Lark、仍慢于 GWS；完整 Lark 诊断 37/40，help RSS 三项未过且样本数未达正式门槛 |
| Linux/Darwin full/race | 待 CI | 阻挡 Ready |

当前本地数据确认了 calendar 构树相对完整树约 94% 的时间下降、七个旧候选代表场景相对固定 main 全部提速，以及 telemetry default/opt-out 基本重合。P0/P1 后工作树尚需重新生成端到端候选；最终结论仍以 clean-head 两平台 CI artifact 为准。
