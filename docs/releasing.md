...
预检包含测试、策略检查、旧正式版命令树兼容检查、全平台打包、npm 安装验证，以及 macOS 环境下的 Homebrew 安装验证。通过后，命令会把 commit、发布仓库、稳定版基线和最终 `dist/` checksums 封存在当前 Git worktree 的本地元数据中。发布：

```bash
dws-release v1.2.3-beta.1 --publish
```

只要源码、远端权限基线和制品哈希没有变化，`--publish` 会复用刚通过的本地封板，不再重复测试和六平台构建；仍会重新查询远端、CI、immutable release 和在途 Release，并在 tag 创建前做最终契约检查。源码、目标仓库、稳定版基线、版本或任一制品发生变化时，封板自动失效并完整重跑。tag 推送成功后本地封板会删除。

命令会在发布前要求再次输入完整版本号。统一入口不提供跳过确认的参数。发布实现是仓库内已提交的 shell 脚本，本地 `dws-release` 只是稳定入口，不需要为每个版本重新生成发布脚本。
